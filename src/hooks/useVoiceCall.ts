...
export function useVoiceCall(nickname: "Vishwa" | "Ammu"): UseVoiceCallReturn {
  ...
  useEffect(() => {
    ...
    socket.on("call-rejected", () => {
      if (cancelledRef.current) return;
      stopRing(); cleanup();
      setCallStatus("idle");      // straight to idle — no white screen for caller either
    });
  ...
  const rejectCall = useCallback(() => {
    stopRing();
    cleanup();                    // clean up mic/PC just in case
    setCallStatus("idle");        // straight to idle — no white screen
