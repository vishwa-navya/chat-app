/**
 * useVoiceCall.ts — FINAL v7 FIXED
 *
 * CRITICAL FIXES:
 * 1. Audio autoplay blocking - Pre-unlock audio element on gesture
 * 2. Remote stream not attaching - Proper ontrack handler
 * 3. Proximity sensor - Use Page Visibility API
 */
...
export function useVoiceCall(nickname: "Vishwa" | "Ammu"): UseVoiceCallReturn {
  ...
  const unlockAudio = () => {
    if (audioElRef.current) {
      console.log("[Audio] Audio element already created");
      return;
    }
    ...
    audio.play()
      ...
      .catch((err) => {
        console.warn("[Audio] Silent audio failed:", err);
      });
  ...
  const playRemoteAudio = useCallback((stream: MediaStream) => {
    ...
    if (!audio) {
      console.warn("[Audio] No pre-created element, creating fallback");
    ...
    console.log("[Audio] Setting srcObject");
  ...
  const buildPC = (): RTCPeerConnection => {
    ...
    if (stream) {
      const tracks = stream.getAudioTracks();
      console.log("[PC] Adding", tracks.length, "audio tracks");
      tracks.forEach(t => {
        pc.addTrack(t, stream);
      });
    } else {
      console.error("[PC] NO local stream");
    }
